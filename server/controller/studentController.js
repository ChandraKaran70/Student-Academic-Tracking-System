...
module.exports = {
        ...
        try {
            ...
            res.status(200).json({
                result: attendence.map(att => {
                    let res = {};
                    res.attendence = (parseFloat(att.lectureAttended / att.totalLecturesByFaculty) * 100).toFixed(2)
    ...
    viewTimetable1: async (req,res,next) => {
        try{
            console.log("user is",req.user)
            const studentTimetable = await Timetable.findOne({department:req.user.department,year:req.user.year,semester:req.user.semester,section:req.user.section})
            console.log("timetable retrieved:",studentTimetable)
            res.status(200).json({TimeTable:studentTimetable})
        }
        catch(err){
            console.log("err in getting timetable",err.message)
        }
    },
    subjectAttendance1: async (req,res,next) => {
        try{
            console.log("in subject Attendance cntrlr")
            const studentId = req.user._id
            const { subjectCode } = req.params
            console.log("id and subCode",studentId,subjectCode)
            const subjectAttendanceList = await SubjectAttendance.find({student:studentId,subjectCode:subjectCode})
            console.log("sub list is",subjectAttendanceList)
            if (!subjectAttendanceList) {
                res.status(400).json({ message: "Subject Attendence not found" })
            }
            res.status(200).json({
                result: subjectAttendanceList.map(subjectAttendance => {
                    var subjectAttendance = {
                        classday:subjectAttendance.classday,
                        classtime:subjectAttendance.classtime,
                        attendanceStatus:subjectAttendance.presentStatus
                    }
            ...
            console.log("err in cntrlr",err.message)
        }
    },
