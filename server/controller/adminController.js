...
const validateParentRegisterInput = require('../validation/parentRegister')
...
module.exports = {
    ...
    addStudent: async (req, res, next) => {
        try {
            ...
            const { name, email, year,semester, fatherName, aadharCard,
    ...
    addTimetable: async (req, res, next) => {
        try {
            ...
            console.log(department,year,section,semester)
            ...
            const timetable1 = await Timetable.findOne({department,year,semester,section})
            console.log("time",timetable1)
            if(!timetable1){
                ...
                console.log("newtable",newTimetable)
                await res.status(200).json(newTimetable)
            }
            else{
                timetable1.avatar = imgResponse.secure_url
                await timetable1.save()
                console.log("modified",timetable1)
                await res.status(200).json(timetable1)
            }
            // }
            // else{
            //     timetable.avatar = imgResponse.secure_url
            ...
            console.log("Error in adding timetable", err.message)
        }
    },
    getAllStudents: async (req, res, next) => {
        try {
    ...
    addParent: async (req, res, next) => {
        try {
            ...
            const { errors, isValid } = validateParentRegisterInput(req.body)
            console.log(errors)
            //Validation
            console.log("in cntrl")
            if (!isValid) {
            ...
            const { name, email,parentMobileNumber,dob,
                aadharCard, gender } = req.body
            ...
            let departmentHelper = "11";
            console.log("befre parents extract")
            const parents = await Parent.find({})
            console.log("parents",parents)
            let helper;
            if (parents.length < 10) {
                helper = "00" + parents.length.toString()
            }
            else if (parents.length < 100 && parents.length > 9) {
                helper = "0" + parents.length.toString()
            }
            else {
                helper = parents.length.toString()
            }
            let hashedPassword;
            hashedPassword = await bcrypt.hash(dob, 10)
            var date = new Date();
            ...
            var registrationNumber = components.join("");
            console.log("befr saving new parent")
            ...
            const students = await Student.find({fatherMobileNumber:parentMobileNumber})
            for (var i = 0; i < students.length; i++) {
                newParent.children.push(students[i]._id)
                await newParent.save()
            }
            console.log("Parent is",newParent)
            await newParent.save()
            res.status(200).json({ result: newParent })
        }
        catch (err) {
            console.log("error", err.message)
            res.status(400).json({ message: `error in adding new parent", ${err.message}` })
        }
    },
    addFaculty: async (req, res, next) => {
        try {
            ...
            const { name, email, designation, department, facultyMobileNumber,
                aadharCard, dob, gender,isHod } = req.body
    ...
    addSubject: async (req, res, next) => {
        try {
            ...
            const { totalLectures,semester, department, subjectCode,
                subjectName, year, registrationNumber } = req.body
            console.log("req bdy is:",req.body)
