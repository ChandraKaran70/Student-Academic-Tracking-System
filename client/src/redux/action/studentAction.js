...
const fetchSubjectAttendenceHelper = (data) => {
    return {
        type: "GET_SUBJECT_ATTENDENCE",
        payload: data
    }
}
...
export const studentLogin = (studentCredential) => {
    return async (dispatch) => {
        try {
            ...
            console.log("after act data",data)
            const { token} = data;
            ...
            // Set current user
            console.log("befr setting student")
            dispatch(setStudent(decoded))
            // console.log("timetable in act",timetable)
...
export const fetchTimetable = () => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Get',
                url: url + "/api/student/checkTimetable"
            })
            console.log("after act",data.TimeTable)
            ...
            console.log("Error in getting timetable is", err.message)
        }
...
export const getSubjectAttendence = (subjectCode) => {
    return async (dispatch) => {
        console.log("getSubject Action start")
        try {
            const { data } = await axios({
                method: 'Get',
                url: url + `/api/student/subjectAttendance1/${subjectCode}`
            })
            dispatch(fetchSubjectAttendenceHelper(data.result))
            console.log("getSubject Action end")
            console.log("result subatt is",data.result)
        }
        catch (err) {
            console.log("Error in subject attendance", err.message)
        }
