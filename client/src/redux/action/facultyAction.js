...
const hodFetchAttendanceHelper = (data)=>{
    return{
        type:"HOD_FETCH_ATTENDANCE",
        payload:data
    }
}


const fetchClassSubjectsHelper = (data)=>{
    return{
        type:"FETCH_CLASS_SUBJECTS",
        payload:data
    }
}
...
const marksFlag = (data)=>{
    return{
        type:"HOD_FETCH_MARKS_FLAG",
        payload:data
    }
}

const setHodFlag =(data)=>{
    return {
        type:"SET_HOD_FLAG",
        payload:data
    }
...
const hodGetAllStudentHelper = (data) => {
    return {
...
export const hodLogin = (hodCredential) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Post',
                url: url + "/api/faculty/hodlogin",
                data: hodCredential
            })
            const { token } = data;
            // Set token to local Storage
...
export const fetchStudents = (department, year, section,registrationNumber) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Post',
                url: url + "/api/faculty/fetchStudents",
                data: { department, year, section,registrationNumber}
            })
...
export const hodFetchStudents = ( department,year, section) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Post',
                url: url + "/api/faculty/hodFetchStudents",
                data: {department, year, section}
            })
            console.log("data",data)
            dispatch(hodFetchAttendanceHelper(data.result))
        }
        catch (err) {
            dispatch({
                type: SET_ERRORS,
...
export const hodGetAllStudent = (searchCredentials) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Post',
            ...
            dispatch(hodGetAllStudentHelper(data.result))
        }
        catch (err) {
            dispatch({
                type: SET_ERRORS,
...
export const hodFetchMarks = (department,year,section,semester,test)=>{
    return async (dispatch) => {
        try{
        const {data} = await axios({
            method:'Post',
            url: url+"/api/faculty/hodFetchMarks",
            data:{department,year,section,semester,test}
        })
        console.log("data is",data)
        dispatch({
            type:"HOD_FETCH_MARKS",
            payload:data.result
        })
        dispatch(marksFlag(true))
        dispatch(fetchClassSubjectsHelper(data.subjects))
    }
    catch (err) {
        dispatch({
            type: SET_ERRORS,
